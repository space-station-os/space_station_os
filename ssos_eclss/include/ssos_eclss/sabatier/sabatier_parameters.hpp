...
struct ReactorParams
{
  ...
  // Thermostatic trim heater (models the ISS Sabatier electric preheater): at
  // flight CO2 rates the exotherm is below the heat loss, so the reactor must
  // be heated to hold the catalyst temperature.
  double trim_heater_max_w;   // max preheater power [W]
  double heater_gain_w_k;     // proportional gain toward setpoint [W/K]
};
