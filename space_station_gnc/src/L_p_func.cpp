...
void casadi_qr(
  const casadi_int * sp_a, const casadi_real * nz_a, casadi_real * x,
  const casadi_int * sp_v, casadi_real * nz_v, const casadi_int * sp_r, casadi_real * nz_r,
  casadi_real * beta,
...
void casadi_qr_mv(
  const casadi_int * sp_v, const casadi_real * v, const casadi_real * beta, casadi_real * x,
  casadi_int tr)
{
...
void casadi_qr_trs(
  const casadi_int * sp_r, const casadi_real * nz_r, casadi_real * x,
  casadi_int tr)
{
...
void casadi_qr_solve(
  casadi_real * x, casadi_int nrhs, casadi_int tr,
...
casadi_int casadi_qr_singular(
  casadi_real * rmin, casadi_int * irmin, const casadi_real * nz_r,
...
void casadi_qr_colcomb(
  casadi_real * v, const casadi_real * nz_r, const casadi_int * sp_r,
...
{
  casadi_int i, j, k;
  casadi_real * rr, * w00 = w + 4, * w01 = w + 16, w02, w03, w04, w05, w06, w07, * w08 = w + 34,
    * w09 = w + 37, * w10 = w + 46;
  ...
  *rr++ = w06;
  /* #18: @8 = (@2*@8) */
  for (i = 0, rr = w08, cs = w08; i < 3; ++i) {
    (*rr++) = (w02 * (*cs++));
  }
  /* #19: (@1[:12:4] = @8) */
  for (rr = w01 + 0, cs = w08; rr != w01 + 12; rr += 4) {
    *rr = *cs++;
  }
  /* #20: @3 = input[1][0] */
  w03 = arg[1] ? arg[1][0] : 0;
  ...
  *rr++ = w07;
  ...
  /* #35: (@1[1:13:4] = @8) */
  for (rr = w01 + 1, cs = w08; rr != w01 + 13; rr += 4) {
    *rr = *cs++;
  }
  /* #36: @5 = 0.57743 */
  ...
  *rr++ = w03;
  ...
  /* #51: (@1[2:14:4] = @8) */
  for (rr = w01 + 2, cs = w08; rr != w01 + 14; rr += 4) {
    *rr = *cs++;
  }
  /* #52: @5 = input[3][0] */
  w05 = arg[3] ? arg[3][0] : 0;
  ...
  *rr++ = w04;
  ...
  /* #68: (@1[3:15:4] = @8) */
  for (rr = w01 + 3, cs = w08; rr != w01 + 15; rr += 4) {
    *rr = *cs++;
  }
  /* #69: @9 =
  [[1, 0, 0],
  ...
  casadi_clear(w10, 9);
  /* #71: @11 = @1' */
  for (i = 0, rr = w11, cs = w01; i < 3; ++i) {
    for (j = 0; j < 4; ++j) {
      rr[i + j * 3] = *cs++;
    }
  }
  /* #72: @10 = mac(@11,@1,@10) */
  for (i = 0, rr = w10; i < 3; ++i) {
    for (j = 0; j < 3; ++j, ++rr) {
      for (k = 0, cs = w11 + j, ct = w01 + i * 4; k < 4; ++k) {
        *rr += cs[k * 3] * *ct++;
      }
    }
  }
  ...
  /* #74: @0 = mac(@1,@9,@0) */
  for (i = 0, rr = w00; i < 3; ++i) {
    for (j = 0; j < 4; ++j, ++rr) {
      for (k = 0, cs = w01 + j, ct = w09 + i * 3; k < 3; ++k) {
        *rr += cs[k * 4] * *ct++;
      }
    }
  }
...
extern "C" CASADI_SYMBOL_EXPORT int pseudoInvFunc(
...
extern "C" CASADI_SYMBOL_EXPORT int pseudoInvFunc_work(
  casadi_int * sz_arg, casadi_int * sz_res,
...
extern "C" CASADI_SYMBOL_EXPORT int pseudoInvFunc_work_bytes(
  casadi_int * sz_arg,
  casadi_int * sz_res,
  casadi_int * sz_iw, casadi_int * sz_w)
{
...
extern "C" CASADI_SYMBOL_EXPORT int hFunc(
  const casadi_real ** arg, casadi_real ** res,
  casadi_int * iw, casadi_real * w, int mem)
{
...
extern "C" CASADI_SYMBOL_EXPORT int hFunc_work(
  casadi_int * sz_arg, casadi_int * sz_res,
  casadi_int * sz_iw, casadi_int * sz_w)
{
  if (sz_arg) {*sz_arg = 7;}
...
extern "C" CASADI_SYMBOL_EXPORT int hFunc_work_bytes(
  casadi_int * sz_arg, casadi_int * sz_res,
  casadi_int * sz_iw, casadi_int * sz_w)
{
  if (sz_arg) {*sz_arg = 7 * sizeof(const casadi_real *);}
