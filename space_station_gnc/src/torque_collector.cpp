...
public:
  ...
  : Node("torque_collector_node")
  {
    ...
    pub_torque_control =
      this->create_publisher<geometry_msgs::msg::Vector3>("gnc/thr_torque_cmd", 1);
...
public:
    PropulsionDevice(
      std::string name_, double Fthrust_,
...
private:
  ...
  void parsing_target_thrust(std::string input, std::string & which, double & throttle_)
  {
    ...
    which = strs[0];
    if (strs[1] == "on") {throttle_ = 1.0;} else if (strs[1] == "off") {throttle_ = 0.0;} else {
