...
: Node("orbit_dynamics_node", options)
{
  ...
  RCLCPP_INFO(
    this->get_logger(), "OrbitDynamicsNode ready. dt=%.3f s, pub=%.3f s, RK4=%s",
...
void OrbitDynamicsNode::on_thrusters(const std_msgs::msg::Float32MultiArray::SharedPtr msg)
{
  if (msg->data.size() != n_thrusters_) {
    RCLCPP_ERROR(
      this->get_logger(), "Expected %zu thruster inputs, got %zu",
...
Eigen::Vector3d OrbitDynamicsNode::calc_leo_acc_j2_term(const Eigen::Vector3d & position)
{
  ...
  double factor = 1.5 * OrbitLib::J2 * OrbitLib::G_ME * OrbitLib::EARTH_RADIUS *
    OrbitLib::EARTH_RADIUS / r5;
...
Eigen::Vector3d OrbitDynamicsNode::calc_leo_acc_air_drag_term(
...
//   - r: ECI position [m]
//   - v: ECI velocity [m/s]
Eigen::Vector3d OrbitDynamicsNode::calc_leo_acceleration(
