...
void display_image_with_sdl(const cv::Mat & image)
{
  ...
  if (window == nullptr) {
    ...
    window = SDL_CreateWindow(
      "Space Station GNC Demo", SDL_WINDOWPOS_UNDEFINED,
      SDL_WINDOWPOS_UNDEFINED, image.cols, image.rows, SDL_WINDOW_SHOWN);
...
public:
  DemoCrisisNode()
  : Node("demo_crisis_node")
  {
    sub_topic_control_media = this->create_subscription<std_msgs::msg::String>(
      "gnc/topic_control_media", 10,
      std::bind(&DemoCrisisNode::callback_control_media, this, std::placeholders::_1));
    sub_angvel_body = this->create_subscription<geometry_msgs::msg::Vector3>(
      "gnc/angvel_body", 1,
      std::bind(&DemoCrisisNode::callback_angvel_body, this, std::placeholders::_1));
...
private:
  ...
  void display_image(const std::string & image_path)
  {
    ...
    if (image.empty()) {
      RCLCPP_ERROR(
        this->get_logger(), "Error: Image %s not found or unable to load.", image_path.c_str());
      return;
    }
...
{
  auto pub_control_media = node->create_publisher<std_msgs::msg::String>(
    "gnc/topic_control_media",
    1);
  auto pub_target_thrust =
    node->create_publisher<std_msgs::msg::String>("gnc/str_target_thrust", 10);
  auto pub_t_fwd_sim = node->create_publisher<std_msgs::msg::Float64>("gnc/t_fwd_sim", 10);

  auto pub_angvel_overwrite = node->create_publisher<geometry_msgs::msg::Vector3>(
    "gnc/angvel_overwrite", 1);
  auto pub_attitude_overwrite = node->create_publisher<geometry_msgs::msg::Quaternion>(
  ...
  while (rclcpp::ok()) {
    ...
    if (mode_demo == "000_init") {
      ...
      if (mode_demo == "102_emergency_reaction") {
        ...
        printf(
          " The Nauka main engine has 3900N to bring the body to zenith, causing pitching down.\n");
