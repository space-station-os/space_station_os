// Copyright 2025 Space Station OS Maintainers
//
...
#ifndef SPACE_STATION_GNC__ORBIT_DYNAMICS_HPP_
#define SPACE_STATION_GNC__ORBIT_DYNAMICS_HPP_
...
constexpr double EARTH_RADIUS = 6378.14 * 1e3;  // [m]
const double EARTH_OMEGA = 7.2921150e-5;  // [rad/s]
...
// Mean motion (rad/s) → semi-major axis [m]
inline double mean_motion_to_a(double n)
{
...
inline void kepler_to_eci(
  ...
  double mu = MU_EARTH)
{
  const double n = n_rev_per_day * Math::TWO_PI / 86400.0;
...
private:
  ...
  double area_{10.0};  // surface area [m^2]
  double cd_{3.5};
  bool use_rk4_{false};
  ...
  std::string topic_thruster_;
  std::string topic_quat_;
  std::string topic_t_fwd_;
  std::string topic_pos_;
  ...
  Eigen::VectorXd thruster_forces_biased_{Eigen::VectorXd::Zero(12)};
  Eigen::Vector3d r_eci_{Eigen::Vector3d::Zero()};
  Eigen::Vector3d v_eci_{Eigen::Vector3d::Zero()};
  Eigen::Vector3d a_eci_{Eigen::Vector3d::Zero()};
  Eigen::Quaterniond q_body_to_eci_{1.0, 0.0, 0.0, 0.0};

  ThrusterMatrix thruster_matrix_;
  ...
  // Helpers
  inline Eigen::Vector3d calc_acc(
    const Eigen::Vector3d & r_eci,
    const Eigen::Vector3d & F_eci) const
  {
  ...
  Eigen::Vector3d calc_leo_acc_air_drag_term(
    const Eigen::Vector3d & position,
    const Eigen::Vector3d & velocity);
  Eigen::Vector3d calc_leo_acceleration(
    const Eigen::Vector3d & position,
    const Eigen::Vector3d & velocity);
...
#endif  // SPACE_STATION_GNC__ORBIT_DYNAMICS_HPP_
