#include "ssos_thermal/nodes/solar_heat_node.hpp"
...
SolarHeatNode::SolarHeatNode()
: Node("solar_heat_node")
{
...
void SolarHeatNode::loadPanelParamsFromROS()
{
  ...
  for (const auto & name : panel_names) {
    ...
    p.area = area;
    p.normal = math3d::Vector3(normal_vec[0], normal_vec[1], normal_vec[2]).normalized();

    panel_map_[name] = p;
    RCLCPP_INFO(this->get_logger(), "Loaded panel %s: alpha=%.2f, area=%.2f",
                name.c_str(), p.absorptivity, p.area);
  }
...
{
  math3d::Vector3 sun_dir(msg->x, msg->y, msg->z);
  sun_dir.normalize();
  ...
  for (const auto & [name, panel] : panel_map_) {
    const double q = computePanelHeat(panel, sun_dir);
...
double SolarHeatNode::computePanelHeat(const PanelParams & panel, const math3d::Vector3 & sun_dir)
{
...
int main(int argc, char ** argv)
{
  ...
  rclcpp::spin(std::make_shared<ssos_thermal::nodes::SolarHeatNode>());
  rclcpp::shutdown();
  return 0;
}
