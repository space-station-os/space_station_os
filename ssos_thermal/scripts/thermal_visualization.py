#!/usr/bin/env python3
"""
ssos_thermal Visualization GUI

PyQt5-based visualization for the thermal network published by
thermal_network_node (/thermal/nodes/state, /thermal/links/flux) and the
coolant tank status published by the legacy space_station_thermal_control
cooling_server (/tcs/ammonia_status) -- same message types and topic names
as the legacy space_station_thermal_control/scripts/thermal_visualization.py
this is ported from, so this viewer works whichever thermal_nodes publisher
(legacy or ssos_thermal) is running.
"""
...
class ThermalLinkItem(QGraphicsLineItem):
    ...
    def _animate_dash(self):
        """Animate dash offset so that dashed area per unit time is proportional to heat flow."""
...
class ThermalVisualizationNode(Node):
    ...
    def __init__(self):
        super().__init__('ssos_thermal_visualization_node')
    ...
    def _on_param_update(self, params):
        # Only updates this node's own color-range state; the GUI re-applies
        # it to every graphics item on its next display tick (see
        # ThermalVisualizationGUI.update_graph), since graphics items live in
        # the GUI, not here.
        for p in params:
            if p.name == "min_temp":
...
class ThermalVisualizationGUI(QWidget):
    ...
    def __init__(self, node):
        super().__init__()
        self.node = node
        self.setWindowTitle("ssos_thermal Visualization")
    ...
    def init_ui(self):
        ...
        legend_text = (
            ...
            "&bull; Red = Hot <br><br>"
            "<b>Node Size:</b><br>"
            "&bull; Larger = Higher heat capacity<br><br>"
            "<b>Arrow Width:</b><br>"
            "&bull; Thicker = Higher heat flow<br><br>"
        )
    ...
    def update_graph(self):
        ...
        # Create/update node items. The temp-color range is re-applied every
        # tick (not just on creation) so a runtime `ros2 param set` on
        # min_temp/max_temp/cold_color/hot_color actually takes visible
        # effect.
    ...
    def update_node_details(self, node_item):
        ...
        self._set_fields(self.node_fields, [
            node_item.name,
            f"{node_item.temperature:.1f} degC",
            f"{node_item.heat_capacity:.1f} J/degC",
    ...
    def update_link_details(self, link_item):
        ...
        self._set_fields(self.link_fields, [
            a,
            b,
            f"{link_item.conductance:.3f} W/degC",
